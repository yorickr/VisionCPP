...
//	namedWindow(windowTitle, CV_WINDOW_AUTOSIZE);
	...
	waitKey(1); // we kunnne niet waitkey(0) gebruiken want dan hangt 't...
} // show16SImage
...
_int16 getEntryNeighbour(const Mat & admin , int x, int y, int nr) {
	...
	default: cout << "ERROR getEntryNeighbour " << endl; // avans really, return dan iets.
	return -1;
	}
} // getEntryNeighbour
...
int labelBLOBs(Mat binaryImage, Mat & labeledImage) {
	...
	// imwrite("b:\\binaryImage.jpg", binaryImage); // waarom in een library dingen schrijven..?
	int row = 1;
	int col = 1;
